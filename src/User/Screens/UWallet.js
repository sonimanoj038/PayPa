...
componentDidMount = async() => {
  ...
  this.setState({type:type,uid:value,isVisible:false},()=>this.GetBalance())
...
{this.state.expdate !==""?<Text style ={{alignSelf:'center',color:'#e46c0b',fontSize:13,paddingHorizontal:5}}> {Moment(this.state.expdate).format('DD,MM, YYYY')}</Text>:null}
</Shimmer>
                              </Right>
             </ListItem>
             </List>
           ...
           onEndReachedThreshold={0.01}
            ...
            <Left>
              ...
              <Text style ={{fontSize:13}}>{item.name}</Text>
                              <Text style ={{fontSize:11,color:'grey'}}>{Moment(item.is_created).format('DD,MM, YYYY H:mma')}</Text>
