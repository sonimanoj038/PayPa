...
componentDidMount = async() => {
 ...
 await this.setState({uid:value,loading2:true,session_id:session_id},()=>this.GetBalance())
...
await fetch('https://www.markupdesigns.org/paypa/api/payment', {
  method: 'POST',
  headers: {
   'Content-Type': 'multipart/form-data',
  },
      ...
      .then((responseJson) => {
       ...
       this.setState({loading:false})
        ...
        if(responseJson.status ==="Failure"){
          ...
          this.setState({loading:false,Alert_Visibility:true,msg:responseJson.msg})
        }}).catch((error) => {
        console.error(error);
      });
    ...
    return (
        ...
        <Header    style={{backgroundColor:'#1c4478'}}>
            <StatusBar barStyle="light-content" backgroundColor="#1c4478"/>
              ...
              <Icon name='md-arrow-back'  style={{color:'white',fontSize:25,left:5}}/>
              </TouchableOpacity></Left>
             <Body style ={{flex:3,alignItems:'center'}}>
              <Text  style = {{color:'white',fontSize:18,fontFamily:'Roboto-Medium'}}>Amount To Pay</Text>
             </Body>
           <Right/>
            </Header>
 ...
 <TouchableOpacity 
     ...
     {""+ this.state.rbal}
        ...
        {this.state.expdate !==""?<Text style ={{color:'white', fontFamily:'Roboto-Light',fontSize:15,textAlign:'center'}}> Your limit balance will expire on
    <Text style ={{color:'#e46c0b', fontFamily:'Roboto-Light',fontSize:15,textAlign:'center'}}>{this.state.expireDate}{" "+Moment(this.state.expdate).format('DD MMMM, YYYY')}</Text></Text>:null}
...
<Image source={require('../../img/common/payment-r.png')} />
           ...
           onChangeText={(amount)=>this.setState({amount})}
            ...
            keyboardType="numeric" placeholder='Enter Amount' maxLength={7} />
          </Item>
          <Mytext></Mytext>
          <Item  rounded style ={[styles.inputitem,{borderColor:"#23528b"}]} >
