...
session_id:'',
 ...
 componentDidMount = async () => {
     ...
     const long =this.props.navigation.state.params.longitude
     this.setState({uid:value,bid:id,latitude:lat,longitude:long,session_id:session_id})
     console.warn(value)
...
{data.latitude && data.longitude !==undefined?<Text style ={{fontSize:15,color:'#c2612d',alignItems:'stretch',padding:5}}>{parseFloat(getPreciseDistance({latitude:this.state.currentLatitude,longitude:this.state.currentLongitude}, {
               latitude: item.latitude,
               longitude: item.longitude,
           },0.01)/1000).toFixed(1)} km away</Text>:null}
<Text></Text>
