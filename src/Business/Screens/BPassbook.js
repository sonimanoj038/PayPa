...
getCat= async () => {
  ...
  getFilterData= async () => {
    ...
    this.setState({uid:value,isVisible:false,type:type},()=>this.PaymentHistory())
...
<ListItem itemHeader first noBorder>
            ...
            <Left>
              ...
              <Text style ={{fontSize:15}}>{item.name}</Text>
                              <Text style ={{fontSize:12,color:'grey'}}>{Moment(item.is_created).format('DD,MM, YYYY H:mma')}</Text>
