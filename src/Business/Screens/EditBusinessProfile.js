...
componentDidMount = async() => {
  ...
  this.setState({mobile:mobile,userid:value,mainLoader:true},()=>{this.FetchData()})
 this. getCat()
...
setProfile = ()=>{
    ...
    submitData = async()=>{
      ...
      // let friday = this.state.friday?'Closed':this.state.Ftime1 +  "-" + this.state.Ftime2
      // let sat = this.state.saturday?'Closed':this.state.Stime1 +  "-" + this.state.Stime2
      // let sunday = this.state.sunday?'Closed':this.state.Sutime1 +  "-" + this.state.Sutime2
      ...
      // formdata.append("fri",friday);
      // formdata.append("sun",sunday);
...
<DeckSwiper
   ref={(c) => this._deckSwiper = c}
          ...
          <Image style={{ height:300,width:'100%',resizeMode:'cover'}}  source={{uri: "https://www.markupdesigns.org/paypa/" + item }}/>
               ...
               <CardItem>
               {/* <Button block info   onPress = {()=>{this.SetCover(item)}}>
               {this.state.coverLoader?<ActivityIndicator
               animating = {this.state.coverLoader}
               color = 'white'
               size={"large"}
               style ={{paddingHorizontal:50,alignItems:'center',width:'100%'}}
              />: <Text style = {{color:'white',fontFamily:'Roborto ',fontSize:18,fontWeight:'800',padding:15}}>Select</Text>}
         </Button> */}
               {/* <CheckBox checked={false} onPress = {()=>{this.SetCover(item)}} /> */}
                 {/* <Icon name="md-heart" style={{ color: '#ED4A6A',fontSize:35 }} /> */}
